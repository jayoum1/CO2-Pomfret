...
export default function ScenarioCarousel({
  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="absolute left-0 top-0 m-3 z-[250] px-2.5 py-1.5 rounded-lg text-xs font-medium bg-white hover:bg-gray-50 border border-gray-200 text-gray-900 shadow-sm pointer-events-auto transition-colors"
      >
        Scenario
      </button>
    )
  ...
  return (
    <div
      data-ui-overlay="true"
      className="absolute left-0 top-0 m-3 w-64 rounded-xl border border-gray-200 bg-white shadow-lg z-[250] overflow-hidden pointer-events-auto"
    >
      <div className="p-3">
        ...
        {isDisturbance && (
          <div className="mb-2">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              />
              <span className="text-sm font-medium text-gray-900 w-7 tabular-nums">{startYear}</span>
            </div>
          </div>
        )}
        ...
        {onReset && (
          <button
            type="button"
            onClick={onReset}
            className="mt-2 w-full py-1.5 text-xs font-medium rounded-lg border border-gray-200 bg-gray-100 text-gray-600 hover:text-gray-900 hover:bg-gray-200 transition-colors"
          >
            Reset scenario
          </button>
        )}
