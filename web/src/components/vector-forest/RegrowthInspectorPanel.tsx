...
export default function RegrowthInspectorPanel({
  ...
  return (
    <div className="flex flex-col h-full overflow-auto bg-[var(--surface)]">
      <div className="p-4 border-b border-[var(--border)] flex items-start justify-between gap-2 shrink-0">
        <div>
          <h2 className="text-lg font-semibold text-[var(--text)]">Sapling Inspector</h2>
          <p className="text-xs text-[var(--text-muted)] mt-0.5">Post-disturbance regrowth</p>
        </div>
        {onClose && (
          <button
            type="button"
      ...
      <div className="p-4 space-y-4 flex-1 min-h-0">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between gap-2">
            ...
            <span className="text-[var(--text)] font-medium">Sapling</span>
          </div>
          <div className="flex justify-between gap-2">
            <span className="text-[var(--text-muted)]">Approx. age</span>
            <span className="text-[var(--text)] font-medium">{Math.max(0, age)} year{age !== 1 ? 's' : ''}</span>
          </div>
          <div className="flex justify-between gap-2">
            <span className="text-[var(--text-muted)]">Appeared at year</span>
            <span className="text-[var(--text)]">{item.spawnYear}</span>
          </div>
          <div className="flex justify-between gap-2">
            <span className="text-[var(--text-muted)]">Origin</span>
            <span className="text-[var(--text)] capitalize">{item.scenarioId.replace(/_/g, ' ')} recovery</span>
          </div>
        </div>
